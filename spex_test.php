...
function getCurveCoordsBetweenSegments($segment1, $segment2)
{
    ...
    $dist_s1_start = vec_abs($s1_start_x - $intersect_x, $s1_start_y - $intersect_y);
    $dist_s1_end = vec_abs($s1_end_x - $intersect_x, $s1_end_y - $intersect_y);

    $dist_s2_start = vec_abs($s2_start_x - $intersect_x, $s2_start_y - $intersect_y);
    $dist_s2_end = vec_abs($s2_end_x - $intersect_x, $s2_end_y - $intersect_y);
    ...
    $dist_tmp_center = vec_abs($vec_near_far_half_x, $vec_near_far_half_y);
    ...
    $cos_a_orig = ($vec_to_near_x * $vec_to_far_x + $vec_to_near_y * $vec_to_far_y) / (vec_abs($vec_to_near_x, $vec_to_near_y) * vec_abs($vec_to_far_x, $vec_to_far_y));
    ...
    // arc/bend radius
    $arc_radius = vec_abs($near_x - $arc_center_x, $near_y - $arc_center_y);
    ...
    if ($arc_radius < CurveCalcLimits::MinimumRadius) {
        deal_with_error(CurveCalcErrorCond::ArcTooTight, "The curve is bending too tight to make a viable bend.");
        return false;
    }
    ...
    [$test_vec_x, $test_vec_y] = vec_rotate($vec_arc_near_x, $vec_arc_near_y, $arc_angle);
    $epsilon = vec_abs($test_vec_x + $arc_center_x - $far_x, $test_vec_y + $arc_center_y - $far_y);

    [$test_vec_x, $test_vec_y] = vec_rotate($vec_arc_near_x, $vec_arc_near_y, -$arc_angle);
    $epsilon_n = vec_abs($test_vec_x + $arc_center_x - $far_x, $test_vec_y + $arc_center_y - $far_y);
...
echo '</pre>';
    ...
    $num_segments = 8;
    // 300
    $num_segments = ceil($curveLength / 300);
...
<svg
    ...
    $straight = vec_abs($far_x - $far_off_x, $far_y - $far_off_y);
    ...
    for ($i = 1; $i <= $num_segments-1; $i++) {
        [$tmp_vec_x, $tmp_vec_y] = vec_rotate($near_x - $arc_center_x, $near_y - $arc_center_y, $alpha_per_segment * $i);
    ...
    $dist_straight_norm_vec = vec_abs($far_off_x - $far_x, $far_off_y - $far_y);
