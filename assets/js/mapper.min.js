...
let cc,cx,cy;cc=this.thingy(vehicle['Rotation'][1],x,y);cx=cc[0];cy=cc[1];path.setAttribute("d","M"+Math.round(x)+","+Math.round(y)+" m-"+(xl / 2)+",-"+(yl / 2)+" h"+(xl-4)+" a2,2 0 0 1 2,2 v"+(yl-4)+" a2,2 0 0 1 -2,2 h-"+(xl-4)+" a2,2 0 0 1 -2,-2 v-"+(yl-4)+" a2,2 0 0 1 2,-2 z");path.setAttribute("fill",fillColor);path.setAttribute("stroke",stroke);path.setAttribute("stroke-width","1");path.setAttribute("transform","rotate("+Math.round(vehicle['Rotation'][1])+", "+Math.round(cx)+", "+Math.round(cy)+")");path.appendChild(title);rollingStockGroup.appendChild(path);}
...
industryLabelGroup.insertAdjacentHTML("beforeend",JSON.parse(industryHtml));this.shapes.push(industryLabelGroup);}
getWaterTowers(){}
