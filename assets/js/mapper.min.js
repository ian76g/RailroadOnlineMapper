...
const playerTable=document.getElementById("playerTable");const editPlayersTable=document.getElementById("editPlayersTable");for(let index=0;index<this.json['Players'].length;index++){const player=this.json['Players'][index];const playerInfoRow=document.createElement("tr");const nameValue=document.createElement("td");const nameTextNode=document.createTextNode(player['Name']);nameValue.appendChild(nameTextNode);playerInfoRow.appendChild(nameValue);const xpValue=document.createElement("td");const xpTextNode=document.createTextNode(player['Xp']);xpValue.appendChild(xpTextNode);playerInfoRow.appendChild(xpValue);const moneyValue=document.createElement("td");const moneyTextNode=document.createTextNode(player['Money']);moneyValue.appendChild(moneyTextNode);playerInfoRow.appendChild(moneyValue);playerTable.appendChild(playerInfoRow);const playerEditInfoRow=document.createElement("tr");const playerEditValue=document.createElement("td");const playerEditTextNode=document.createTextNode(player['Name']);playerEditValue.appendChild(playerEditTextNode);playerEditInfoRow.appendChild(playerEditValue);const playerEditXpValue=document.createElement("td");const playerEditXpInput=document.createElement("input");playerEditXpInput.size=5;playerEditXpInput.maxLength=15;playerEditXpInput.name="xp_"+index;playerEditXpInput.value=player['Xp'];playerEditXpValue.appendChild(playerEditXpInput);playerEditInfoRow.appendChild(playerEditXpValue);const playerEditMoneyValue=document.createElement("td");const playerEditMoneyInput=document.createElement("input");playerEditMoneyInput.size=5;playerEditMoneyInput.maxLength=15;playerEditMoneyInput.name="money_"+index;playerEditMoneyInput.value=player['Money'];playerEditMoneyValue.appendChild(playerEditMoneyInput);playerEditInfoRow.appendChild(playerEditMoneyValue);const playerEditNearValue=document.createElement("td");let playerEditNearTextNode=document.createTextNode("Unknown");if('Industries'in this.json){playerEditNearTextNode=document.createTextNode(this._nearestIndustry(player['Location'],this.json['Industries']));}
...
getTracksAndBeds(){const tracksGroup=document.createElementNS(this.svgNS,"g");const bedsGroup=document.createElementNS(this.svgNS,"g");const maxSlopeLabelGroup=document.createElementNS(this.svgNS,"g");tracksGroup.setAttribute("class","tracks");bedsGroup.setAttribute("class","beds");const slopeLabelGroup=Array(document.createElementNS(this.svgNS,"g"),document.createElementNS(this.svgNS,"g"),document.createElementNS(this.svgNS,"g"),document.createElementNS(this.svgNS,"g"),document.createElementNS(this.svgNS,"g"));slopeLabelGroup[0].setAttribute("class","slopeLabel0");slopeLabelGroup[1].setAttribute("class","slopeLabel1");slopeLabelGroup[2].setAttribute("class","slopeLabel2");slopeLabelGroup[3].setAttribute("class","slopeLabel3");slopeLabelGroup[4].setAttribute("class","slopeLabel4");const slopeLabelSilly=document.createElementNS(this.svgNS,"text");const textNodeSilly=document.createTextNode("");slopeLabelSilly.setAttribute("x","0");slopeLabelSilly.setAttribute("y","0");slopeLabelSilly.appendChild(textNodeSilly);slopeLabelGroup[0].appendChild(slopeLabelSilly);slopeLabelGroup[1].appendChild(slopeLabelSilly);slopeLabelGroup[2].appendChild(slopeLabelSilly);slopeLabelGroup[3].appendChild(slopeLabelSilly);slopeLabelGroup[4].appendChild(slopeLabelSilly);maxSlopeLabelGroup.setAttribute("class","maxSlopeLabel");const drawOrder={};if(this.config.ironOverWood){drawOrder[1]=[1,15,'darkkhaki'];drawOrder[2]=[2,15,'darkkhaki'];drawOrder[5]=[5,15,'darkgrey'];drawOrder[6]=[6,15,'darkgrey'];drawOrder[3]=[3,15,'orange'];drawOrder[7]=[7,15,'lightblue'];drawOrder[4]=[4,3,'black'];drawOrder[0]=[0,3,'black'];}else{drawOrder[1]=[1,15,'darkkhaki'];drawOrder[2]=[2,15,'darkkhaki'];drawOrder[5]=[5,15,'darkgrey'];drawOrder[6]=[6,15,'darkgrey'];drawOrder[7]=[7,15,'lightblue'];drawOrder[3]=[3,15,'orange'];drawOrder[4]=[4,3,'black'];drawOrder[0]=[0,3,'black'];}
let slopecoords=[0,0];if('Splines'in this.json){let splineIndex=-1;for(const spline of this.json['Splines']){splineIndex++;let type=spline['Type'];let entry=drawOrder[type];const[,strokeWidth,stroke]=entry;let segments=spline['Segments'];if([1,2,5,6,3,7].indexOf(type)>-1){const bedSegment=document.createElementNS(this.svgNS,'path');let path='';let tool='';for(const segment of segments){if(segment['Visible']!==1){tool='M';}else{tool='L';}
let xStart=(this.imx-((segment['LocationStart']['X']-this.minX)/ 100*this.scale));let yStart=(this.imy-((segment['LocationStart']['Y']-this.minY)/ 100*this.scale));let xEnd=(this.imx-((segment['LocationEnd']['X']-this.minX)/ 100*this.scale));let yEnd=(this.imy-((segment['LocationEnd']['Y']-this.minY)/ 100*this.scale));let xCenter=(this.imx-((segment['LocationCenter']['X']-this.minX)/ 100*this.scale));let yCenter=(this.imy-((segment['LocationCenter']['Y']-this.minY)/ 100*this.scale));if(path===''){path='M '+xStart+','+yStart+' ';path+=tool+' '+xEnd+','+yEnd+' ';}else{path+=tool+' '+xEnd+','+yEnd+' ';}}
bedSegment.setAttribute("d",path);bedSegment.setAttribute("fill",'none');bedSegment.setAttribute("stroke",stroke);bedSegment.setAttribute("stroke-width",strokeWidth.toString());if(type===3){bedSegment.setAttribute("class","wooden");}else if(type===7){bedSegment.setAttribute("class","iron");}
bedsGroup.appendChild(bedSegment);}else{let segmentIndex=-1;for(const segment of segments){segmentIndex++;if(segment['Visible']!==1){continue}
let xStart=(this.imx-((segment['LocationStart']['X']-this.minX)/ 100*this.scale));let yStart=(this.imy-((segment['LocationStart']['Y']-this.minY)/ 100*this.scale));let xEnd=(this.imx-((segment['LocationEnd']['X']-this.minX)/ 100*this.scale));let yEnd=(this.imy-((segment['LocationEnd']['Y']-this.minY)/ 100*this.scale));let xCenter=(this.imx-((segment['LocationCenter']['X']-this.minX)/ 100*this.scale));let yCenter=(this.imy-((segment['LocationCenter']['Y']-this.minY)/ 100*this.scale));const trackSegment=document.createElementNS(this.svgNS,'line');trackSegment.setAttribute("x1",xStart.toString());trackSegment.setAttribute("y1",yStart.toString());trackSegment.setAttribute("x2",xEnd.toString());trackSegment.setAttribute("y2",yEnd.toString());trackSegment.setAttribute("sp",splineIndex);trackSegment.setAttribute("se",segmentIndex);trackSegment.setAttribute("stroke",stroke);trackSegment.setAttribute('onclick','my_function('+splineIndex+','+segmentIndex+')');trackSegment.setAttribute("stroke-width",strokeWidth.toString());tracksGroup.appendChild(trackSegment);let distance=Math.sqrt(Math.pow(segment['LocationEnd']['X']-segment['LocationStart']['X'],2)+
...
this.shapes.push(bedsGroup);this.shapes.push(tracksGroup);this.shapes.push(slopeLabelGroup[0]);this.shapes.push(slopeLabelGroup[1]);this.shapes.push(slopeLabelGroup[2]);this.shapes.push(slopeLabelGroup[3]);this.shapes.push(slopeLabelGroup[4]);}
...
const cartOptions={'handcar':[this.engineRadius,'white'],'porter_040':[this.engineRadius,'black'],'porter_042':[this.engineRadius,'black'],'eureka':[this.engineRadius,'black'],'eureka_tender':[this.engineRadius,'black'],'climax':[this.engineRadius,'black'],'heisler':[this.engineRadius,'black'],'class70':[this.engineRadius,'black'],'class70_tender':[this.engineRadius,'black'],'cooke260':[this.engineRadius,'black'],'cooke260_tender':[this.engineRadius,'black'],'flatcar_logs':[this.engineRadius / 3,'indianred','red'],'flatcar_cordwood':[this.engineRadius / 3*2,'orange','orangered'],'flatcar_stakes':[this.engineRadius / 3*2,'greenyellow','green'],'flatcar_hopper':[this.engineRadius / 3*2,'rosybrown','brown'],'boxcar':[this.engineRadius / 3*2,'mediumpurple','purple'],'flatcar_tanker':[this.engineRadius / 3*2,'lightgray','dimgray'],}
let index=0;for(const vehicle of this.json['Frames']){const x=(this.imx-((vehicle['Location'][0]-this.minX)/ 100*this.scale));const y=(this.imy-((vehicle['Location'][1]-this.minY)/ 100*this.scale));if(['porter_040','porter_042','eureka','climax','heisler','class70','cooke260'].indexOf(vehicle['Type'])>=0){const yl=(this.engineRadius / 3)*2;const xl=(this.engineRadius / 2)*2;const path=document.createElementNS(this.svgNS,"path");path.setAttribute("transform","rotate("+Math.round(vehicle['Rotation'][1])+", "+x+", "+y+")");path.setAttribute("d","M"+(x-(this.engineRadius / 2))+","+y+" l "+(xl / 3)+","+(yl / 2)+" l "+(xl / 3*2)+",0 l 0,-"+yl+" l -"+(xl / 3*2)+",0 z");path.setAttribute("fill","purple");path.setAttribute("stroke","black");path.setAttribute("stroke-width","2");rollingStockGroup.appendChild(path);}else{const yl=(this.engineRadius / 3)*2;let xl=this.engineRadius;if(vehicle['Type'].toLowerCase().indexOf('tender')!==-1){xl=xl / 3*2;}
let fillColor=cartOptions[vehicle['Type']][1];if(typeof vehicle['Freight']!=='undefined'&&typeof vehicle['Freight']['Amount']!=='undefined'&&vehicle['Freight']['Amount']>0&&cartOptions[vehicle['Type']][2]!==undefined){fillColor=cartOptions[vehicle['Type']][2];}
const title=document.createElementNS(this.svgNS,"title");title.textContent=vehicle['Name'].replace(/<\/?[^>]+(>|$)/g,"")+" "+vehicle['Number'].replace(/<\/?[^>]+(>|$)/g,"");if(typeof vehicle['Freight']!=='undefined'&&typeof vehicle['Freight']['Amount']!=='undefined'&&vehicle['Freight']['Amount']===0){title.textContent+=" (empty)";}else{if(typeof vehicle['Freight']!=='undefined'&&typeof vehicle['Freight']['Type']!=='undefined'){title.textContent+=" ("+vehicle['Freight']['Type']+" x"+vehicle['Freight']['Amount']+")";}}
const path=document.createElementNS(this.svgNS,"path");path.setAttribute("d","M"+Math.round(x)+","+Math.round(y)+" m-"+(xl / 2)+",-"+(yl / 2)+" h"+(xl-4)+" a2,2 0 0 1 2,2 v"+(yl-4)+" a2,2 0 0 1 -2,2 h-"+(xl-4)+" a2,2 0 0 1 -2,-2 v-"+(yl-4)+" a2,2 0 0 1 2,-2 z");path.setAttribute("fill",fillColor);path.setAttribute("stroke","black");path.setAttribute("stroke-width","1");path.setAttribute("transform","rotate("+Math.round(vehicle['Rotation'][1])+", "+Math.round(x)+", "+Math.round(y)+")");path.appendChild(title);rollingStockGroup.appendChild(path);}
