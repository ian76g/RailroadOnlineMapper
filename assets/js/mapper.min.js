...
let slopecoords=[0,0];if('Splines'in this.json){for(const spline of this.json['Splines']){let type=spline['Type'];let entry=drawOrder[type];const[,strokeWidth,stroke]=entry;let segments=spline['Segments'];if([1,2,5,6,3,7].indexOf(type)>-1){const bedSegment=document.createElementNS(this.svgNS,'path');let path='';let tool='';for(const segment of segments){if(segment['Visible']!==1){tool='M';}else{tool='L';}
