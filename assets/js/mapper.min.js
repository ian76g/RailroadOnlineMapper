...
thingy(angle,x,y){return[x,y];x+=Math.cos(this.deg2rad(angle))*2;y+=Math.sin(this.deg2rad(angle));return[x,y];}
drawSVG(htmlElement,industryHtml){this.svgTag=document.getElementById(htmlElement);this.getReplantableTrees();this.getTracksAndBeds();this.getSwitches();this.getTurntables();this.getIndustries(industryHtml);this.getWaterTowers();this.getRollingStock();this.populatePlayerTable();for(const shape of this.shapes){this.svgTag.appendChild(shape);}}
...
const playerGroup=document.createElementNS(this.svgNS,"g");playerGroup.setAttribute("class","players_default");const editPlayersTable=document.getElementById("editPlayersTable");for(let index=0;index<this.json['Players'].length;index++){let player=this.json['Players'][index];let playerEditInfoRow=document.createElement("tr");let playerEditValue=document.createElement("td");let playerEditTextNode=document.createTextNode(player['Name']);playerEditValue.appendChild(playerEditTextNode);playerEditInfoRow.appendChild(playerEditValue);let playerEditXpValue=document.createElement("td");let playerEditXpInput=document.createElement("input");playerEditXpInput.size=5;playerEditXpInput.maxLength=15;playerEditXpInput.name="xp_"+index;playerEditXpInput.value=player['Xp'];playerEditXpValue.appendChild(playerEditXpInput);playerEditInfoRow.appendChild(playerEditXpValue);let playerEditMoneyValue=document.createElement("td");let playerEditMoneyInput=document.createElement("input");playerEditMoneyInput.size=5;playerEditMoneyInput.maxLength=15;playerEditMoneyInput.name="money_"+index;playerEditMoneyInput.value=player['Money'];playerEditMoneyValue.appendChild(playerEditMoneyInput);playerEditInfoRow.appendChild(playerEditMoneyValue);let playerEditNearValue=document.createElement("td");let playerEditNearTextNode=document.createTextNode("Unknown");if('Industries'in this.json){playerEditNearTextNode=document.createTextNode(this._nearestIndustry(player['Location'],this.json['Industries'],false));}
playerEditNearValue.appendChild(playerEditNearTextNode);playerEditInfoRow.appendChild(playerEditNearValue);let playerEditDeleteValue=document.createElement("td");if(index===0){let playerEditDeleteInput=document.createTextNode("Unknown");playerEditDeleteValue.appendChild(playerEditDeleteInput);}else{let playerEditDeleteInput=document.createElement("input");playerEditDeleteInput.type="checkbox"
playerEditDeleteInput.name="deletePlayer_"+index;playerEditDeleteValue.appendChild(playerEditDeleteInput);}
if(player['Location']===undefined||player['Location'][0]===undefined){player['Location']=[];player['Location'][0]=0;player['Location'][1]=0;}
let px=(this.imx-((player['Location'][0]-this.minX)/ 100*this.scale));let py=(this.imy-((player['Location'][1]-this.minY)/ 100*this.scale));let ps=this.polarToCartesian(px,py,3,20+player['Rotation']);let pe=this.polarToCartesian(px,py,3,340+player['Rotation']);let playerCircle=document.createElementNS(this.svgNS,"path");let path='M '+ps.x+' '+ps.y+' A 3 3 0 1 0 '+pe.x+' '+pe.y;path+=' A 1 1 0 1 0 '+ps.x+' '+ps.y;playerCircle.setAttribute("d",path);playerCircle.setAttribute("fill","pink");playerCircle.setAttribute("stroke","black");playerCircle.setAttribute("stroke-width","1");let title=document.createElementNS(this.svgNS,"title");title.textContent=player['Name'].replace(/<\/?[^>]+(>|$)/g,"");playerCircle.appendChild(title);playerGroup.appendChild(playerCircle);playerEditInfoRow.appendChild(playerEditDeleteValue);editPlayersTable.appendChild(playerEditInfoRow);}
this.shapes.push(playerGroup);}
getTracksAndBeds(){const tracksGroup=document.createElementNS(this.svgNS,"g");const bedsGroup=document.createElementNS(this.svgNS,"g");const ironBridgeGroup=document.createElementNS(this.svgNS,"g");const maxSlopeLabelGroup=document.createElementNS(this.svgNS,"g");tracksGroup.setAttribute("class","tracks");bedsGroup.setAttribute("class","beds");ironBridgeGroup.setAttribute("class","ironOverWood");maxSlopeLabelGroup.setAttribute("class","maxSlopeLabel");const slopeLabelGroup=Array(document.createElementNS(this.svgNS,"g"),document.createElementNS(this.svgNS,"g"),document.createElementNS(this.svgNS,"g"),document.createElementNS(this.svgNS,"g"),document.createElementNS(this.svgNS,"g"),document.createElementNS(this.svgNS,"g"),document.createElementNS(this.svgNS,"g"),document.createElementNS(this.svgNS,"g"),document.createElementNS(this.svgNS,"g"));slopeLabelGroup[0].setAttribute("class","slopeLabel0");slopeLabelGroup[1].setAttribute("class","slopeLabel1");slopeLabelGroup[2].setAttribute("class","slopeLabel2");slopeLabelGroup[3].setAttribute("class","slopeLabel3");slopeLabelGroup[4].setAttribute("class","slopeLabel4");slopeLabelGroup[5].setAttribute("class","slopeLabel5");slopeLabelGroup[6].setAttribute("class","slopeLabel6");slopeLabelGroup[7].setAttribute("class","slopeLabel7");slopeLabelGroup[8].setAttribute("class","slopeLabel8");const slopeLabelSilly=document.createElementNS(this.svgNS,"text");const textNodeSilly=document.createTextNode("");slopeLabelSilly.setAttribute("x","0");slopeLabelSilly.setAttribute("y","0");slopeLabelSilly.appendChild(textNodeSilly);slopeLabelGroup[0].appendChild(slopeLabelSilly);slopeLabelGroup[1].appendChild(slopeLabelSilly);slopeLabelGroup[2].appendChild(slopeLabelSilly);slopeLabelGroup[3].appendChild(slopeLabelSilly);slopeLabelGroup[4].appendChild(slopeLabelSilly);slopeLabelGroup[5].appendChild(slopeLabelSilly);slopeLabelGroup[6].appendChild(slopeLabelSilly);slopeLabelGroup[7].appendChild(slopeLabelSilly);slopeLabelGroup[8].appendChild(slopeLabelSilly);const drawOrder={};drawOrder[1]=[1,15,'darkkhaki'];drawOrder[2]=[2,15,'darkkhaki'];drawOrder[5]=[5,15,'darkgrey'];drawOrder[6]=[6,15,'darkgrey'];drawOrder[3]=[3,15,'orange'];drawOrder[7]=[7,15,'lightblue'];drawOrder[4]=[4,2,'black'];drawOrder[0]=[0,2,'black'];let slopecoords=[0,0];let order=[1,2,5,6,3,7,4,0];var oi;for(oi=0;oi<8;oi++){if('Splines'in this.json){let splineIndex=-1;for(const spline of this.json['Splines']){splineIndex++;let type=spline['Type'];if(type!==order[oi])continue;let entry=drawOrder[type];let[,strokeWidth,stroke]=entry;let multiplier=1;if([1,2,5,6].indexOf(type)>-1){let x=this.getZDistanceToNearestTrack(spline['Segments'][0]);if(x>0){multiplier=Math.round(x / 180);}}
...
if(O!==null){let OP={}
OP.X=O.x;OP.Y=O.y;let radius=Math.round(this._dist(OP,segment['LocationStart'],true)/ 100);let index=8;if(radius<120){index=7;}
...
let cc,cx,cy;cc=this.thingy(vehicle['Rotation'][1],x,y);cx=cc[0];cy=cc[1];const width=1.9327;const length=cartOptions[vehicle['Type']][0];path.setAttribute("width",length*1.8);path.setAttribute("height",width*3);path.setAttribute("x",x-(path.getAttribute("width")/ 2));path.setAttribute("y",y-(path.getAttribute("height")/ 2));path.setAttribute("rx",1.5)
path.setAttribute("ry",1.5)
path.setAttribute("fill",fillColor);path.setAttribute("stroke",stroke);path.setAttribute("stroke-width","1");path.setAttribute("transform","rotate("+Math.round(vehicle['Rotation'][1])+", "+x+", "+y+")");path.setAttribute("fill",fillColor);path.appendChild(title);rollingStockGroup.appendChild(path);}
...
const rollingStockInfoRow=document.createElement("tr");const typeValue=document.createElement("td");const a=document.createElement('a');const typeImage=document.createElement("img");typeImage.src="/assets/images/"+vehicle['Type']+".png";a.appendChild(typeImage);a.href="javascript:zoomTo("+(400000-(vehicle['Location'][0]+200000))/ 400000+","+(400000-(vehicle['Location'][1]+200000))/ 400000+")";typeValue.appendChild(a);rollingStockInfoRow.appendChild(typeValue);const nameValue=document.createElement("td");const nameTextInput=document.createElement("input");nameTextInput.size=5;nameTextInput.maxLength=22;nameTextInput.name="name_"+index;nameTextInput.value=vehicle['Name'].replace(/<\/?[^>]+(>|$)/g,"");nameValue.appendChild(nameTextInput);rollingStockInfoRow.appendChild(nameValue);const numberValue=document.createElement("td");const numberTextInput=document.createElement("input");numberTextInput.size=5;numberTextInput.maxLength=4;numberTextInput.name="number_"+index;numberTextInput.value=vehicle['Number'].replace(/<\/?[^>]+(>|$)/g,"");numberValue.appendChild(numberTextInput);rollingStockInfoRow.appendChild(numberValue);const nearValue=document.createElement("td");let nearTextNode=document.createTextNode("Unknown");if('Industries'in this.json){nearTextNode=document.createTextNode(this._nearestIndustry(vehicle['Location'],this.json['Industries']));}
...
rollingStockInfoRow.appendChild(cargoValue);const amountValue=document.createElement("td");const amountTextInput=document.createElement("input");amountTextInput.name=amountString+index;amountTextInput.value=amount;amountTextInput.size=2;amountTextInput.maxLength=4;amountValue.appendChild(amountTextInput);rollingStockInfoRow.appendChild(amountValue);if(['porter_040','porter_042','eureka','eureka_tender','climax','heisler','class70','class70_tender','cooke260','cooke260_tender'].indexOf(vehicle['Type'])>=0){rollingStockTable.appendChild(rollingStockInfoRow);}else{rollingStockTable2.appendChild(rollingStockInfoRow);}
index++;}
this.shapes.push(rollingStockGroup);}
getIndustries(industryHtml){if(!('Industries'in this.json)){return}
let x;let y;let path;const industryLabelGroup=document.createElementNS(this.svgNS,"g");industryLabelGroup.setAttribute("class","industryLabel");const industriesTable=document.getElementById("industriesTable");industryLabelGroup.insertAdjacentHTML("beforeend",industryHtml);let index=0;for(const industry of this.json['Industries']){let name='';let rotation=0;let xoff=0;let yoff=0;let pis=[];let pos=[];let indLength;let indWidth;x=(this.imx-((industry['Location'][0]-this.minX)/ 100*this.scale));y=(this.imy-((industry['Location'][1]-this.minY)/ 100*this.scale));switch(industry['Type']){case 1:industry['EductsStored'].pop();industry['EductsStored'].pop();industry['EductsStored'].pop();industry['EductsStored'].pop();pos=['logs_p.svg','cordwood_p.svg','cordwood_p.svg','logs_p.svg'];name="Logging Camp";rotation=0;xoff=-70;yoff=-30;break;case 2:name='Sawmill';industry['EductsStored'].pop();industry['EductsStored'].pop();industry['EductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();pos=['lumber_p.svg','beams_p.svg'];pis=['logs_p.svg'];xoff=-35;yoff=-15;rotation=45;break;case 3:name='Smelter';industry['EductsStored'].pop();industry['EductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();pos=['iron_p.svg','rails_p.svg'];pis=['cordwood_p.svg','ironore_p.svg'];rotation=90;break;case 4:name='Ironworks';industry['EductsStored'].pop();industry['EductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();pos=['pipes_p.svg','tools_p.svg'];pis=['iron_p.svg','coal_p.svg'];rotation=industry['Rotation'][1];xoff=-50;break;case 5:name='Oilfield';industry['EductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();pis=['pipes_p.svg','beams_p.svg','tools_p.svg'];pos=['oil_p.svg'];rotation=0;break;case 6:name='Refinery';industry['EductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();pis=['oil_p.svg','pipes_p.svg','lumber_p.svg'];pos=['barrels_p.svg','barrels_p.svg'];rotation=0;break;case 7:name='Coal Mine';industry['EductsStored'].pop();industry['EductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();pis=['beams_p.svg','rails_p.svg'];pos=['coal_p.svg'];rotation=industry['Rotation'][1]-90;xoff=-60;yoff=-30;break;case 8:name='Iron Mine';industry['EductsStored'].pop();industry['EductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();pis=['lumber_p.svg','beams_p.svg'];pos=['ironore_p.svg'];rotation=industry['Rotation'][1]+90;yoff=+20;xoff=-90;break;case 9:name='Freight Depot';industry['EductsStored'].pop();industry['EductsStored'].pop();industry['EductsStored'].pop();industry['EductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();rotation=industry['Rotation'][1];xoff=-50;rotation=industry['Rotation'][1];break;case 10:industry['EductsStored'].pop();industry['EductsStored'].pop();industry['EductsStored'].pop();pis=['cordwood_p.svg'];name='F#'+index;rotation=industry['Rotation'][1]+90;path=document.createElementNS(this.svgNS,"path");path.setAttribute("transform","rotate("+Math.round(industry['Rotation'][1])+", "+x+", "+y+")");path.setAttribute("d","M"+x+","+y+" m-18,-15 l10,0 l0,30 l-10,0 z");path.setAttribute("fill","orange");path.setAttribute("stroke","brown");industryLabelGroup.appendChild(path);xoff=-20;yoff=+18;break;case 11:case 12:case 13:case 14:let fill=[];fill[11]='lightblue';fill[12]='gold';fill[13]='red';fill[14]='brown';industry['ProductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();industry['ProductsStored'].pop();industry['EductsStored'].pop();industry['EductsStored'].pop();industry['EductsStored'].pop();industry['EductsStored'].pop();name='H#'+index;rotation=industry['Rotation'][1];path=document.createElementNS(this.svgNS,"path");path.setAttribute("transform","rotate("+Math.round(industry['Rotation'][1])+", "+x+", "+y+")");path.setAttribute("d","M"+x+","+y+" m-45,-12 l45,0 l0,24 l-45,0 z");path.setAttribute("fill",fill[industry['Type']]);path.setAttribute("stroke","brown");industryLabelGroup.appendChild(path);xoff=-45;yoff=+6;break;default:console.log("Unknown industry: "+JSON.stringify(industry));}
const industryLabel=document.createElementNS(this.svgNS,"text");const textNode=document.createTextNode(name);industryLabel.setAttribute("x",x+xoff);industryLabel.setAttribute("y",y+yoff);industryLabel.setAttribute("transform","rotate("+rotation+", "+x+", "+y+")");industryLabel.appendChild(textNode);industryLabelGroup.appendChild(industryLabel);let c=document.createElementNS(this.svgNS,"circle");c.setAttribute("cx",x);c.setAttribute("cy",y);c.setAttribute("r","2");c.setAttribute("stroke","red");c.setAttribute("stroke-width","1");c.setAttribute("fill","none");industryLabelGroup.appendChild(c);const eductRow=document.createElement("tr");eductRow.setAttribute("class","export__educts");const eductnameColumn=document.createElement("td");const eductnameColumnText=document.createTextNode(name+" Input");eductnameColumn.appendChild(eductnameColumnText);eductRow.appendChild(eductnameColumn);for(let i=0;i<industry['EductsStored'].length;i++){const itemColumn=document.createElement("td");const itemInput=document.createElement("input");itemInput.size=5;itemInput.maxLength=15;itemInput.name="educt"+i+"_"+index;itemInput.value=industry['EductsStored'][i];if(typeof pis[i]!=="undefined"){const itemImage=document.createElement("img");itemImage.setAttribute("style","float:right");itemImage.setAttribute("src","/assets/images/"+pis[i]);itemImage.setAttribute("height","30");itemColumn.appendChild(itemImage);}
itemColumn.appendChild(itemInput);eductRow.appendChild(itemColumn);}
industriesTable.appendChild(eductRow);const productRow=document.createElement("tr");productRow.setAttribute("class","export__products");const productNameColumn=document.createElement("td");const productNameColumnText=document.createTextNode(name+" Output");productNameColumn.appendChild(productNameColumnText);productRow.appendChild(productNameColumn);for(let i=0;i<industry['ProductsStored'].length;i++){const itemColumn=document.createElement("td");const itemInput=document.createElement("input");itemInput.size=5;itemInput.maxLength=15;itemInput.name="product"+i+"_"+index;itemInput.value=industry['ProductsStored'][i];if(typeof pos[i]!=="undefined"){const itemImage=document.createElement("img");itemImage.setAttribute("style","float:right");itemImage.setAttribute("src","/assets/images/"+pos[i]);itemImage.setAttribute("height","30");itemColumn.appendChild(itemImage);}
itemColumn.appendChild(itemInput);productRow.appendChild(itemColumn);}
industriesTable.appendChild(productRow);index+=1;}
if(('Watertowers'in this.json)){for(const tower of this.json['Watertowers']){const x=this.imx-((tower['Location'][0]-this.minX)/ 100*this.scale);const y=this.imy-((tower['Location'][1]-this.minY)/ 100*this.scale);const waterTower=document.createElementNS(this.svgNS,"path");waterTower.setAttribute("transform","rotate("+Math.round(tower['Rotation'][1])+", "+x+", "+y+")");waterTower.setAttribute("d","M"+x+","+y+" m -5,-5 l10,0 l0,3 l3,0 l0,4 l-3,0 l0,3 l-10,0 z");waterTower.setAttribute("fill","lightblue");waterTower.setAttribute("stroke","black");waterTower.setAttribute("stroke-width","1");industryLabelGroup.appendChild(waterTower);const waterTowerCircle=document.createElementNS(this.svgNS,"circle");waterTowerCircle.setAttribute("cx",x.toString());waterTowerCircle.setAttribute("cy",y.toString());waterTowerCircle.setAttribute("r","3");waterTowerCircle.setAttribute("fill","blue");industryLabelGroup.appendChild(waterTowerCircle);}}
this.shapes.push(industryLabelGroup);}
getWaterTowers(){}
getZDistanceToNearestTrack(segment){const treeX=Math.floor((200000+segment['LocationCenter']['X'])/ 100000);const treeY=Math.floor((200000+segment['LocationCenter']['Y'])/ 100000);const tree=[segment['LocationCenter']['X'],segment['LocationCenter']['Y'],segment['LocationCenter']['Z']];let minDistanceToSomething=undefined;try{for(const spline of this.json['Splines']){if(spline['Type']!==0&&spline['Type']!==4)continue;for(const segment of spline['Segments']){if(segment['CX'].indexOf(treeX)===-1)continue;if(segment['CY'].indexOf(treeY)===-1)continue;if(segment['LocationCenter']['X']<tree[0]-6000){continue;}
...
return minDist;}
_nearestIndustry(coords,industryCoords,log=false){let minDist=800000;let ind=0;for(const i of industryCoords){if(i['Type']<10){let d=this._dist(i['Location'],coords,true);if(log){}
if(d<minDist){minDist=d;ind=i['Type'];}}}
...
return name;}
_dist(coordsIn,coords2In,flat=false){if(coords2In===undefined){return 9999999999;}
const coords=JSON.parse(JSON.stringify(coordsIn));const coords2=JSON.parse(JSON.stringify(coords2In));let distance;if('X'in coords){if('X'in coords2){if(flat===true){coords['Z']=coords2['Z']=0;}
distance=Math.sqrt(Math.pow(coords['X']-coords2['X'],2)+
Math.pow(coords['Y']-coords2['Y'],2)+
Math.pow(coords['Z']-coords2['Z'],2));}else{if(flat===true){coords['Z']=coords2[2]=0;}
distance=Math.sqrt(Math.pow(coords['X']-coords2[0],2)+
Math.pow(coords['Y']-coords2[1],2)+
Math.pow(coords['Z']-coords2[2],2));}}else{if('X'in coords2){if(flat===true){coords[2]=coords2['Z']=0;}
distance=Math.sqrt(Math.pow(coords[0]-coords2['X'],2)+
Math.pow(coords[1]-coords2['Y'],2)+
Math.pow(coords[2]-coords2['Z'],2));}else{if(flat===true){coords[2]=coords2[2]=0;}
...
return result;};polarToCartesian(centerX,centerY,radius,angleInDegrees){var angleInRadians=(angleInDegrees-90)*Math.PI / 180.0;return{x:centerX+(radius*Math.cos(angleInRadians)),y:centerY+(radius*Math.sin(angleInRadians))};}}
Deprecated: strpos(): Non-string needles will be interpreted as strings in the future. Use an explicit chr() call to preserve the current behavior in C:\Users\Sebastian\PhpstormProjects\GVASONLINE\utils\Minifier.php on line 184
