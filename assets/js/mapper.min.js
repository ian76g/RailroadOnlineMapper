...
{const treeX=Math.floor((200000+segment['LocationCenter']['X'])/ 100000);const treeY=Math.floor((200000+segment['LocationCenter']['Y'])/ 100000);const tree=[segment['LocationCenter']['X'],segment['LocationCenter']['Y'],segment['LocationCenter']['Z']];let minDistanceToSomething=80000000;try{for(const spline of this.json['Splines']){if(spline['Type']!==0&&spline['Type']!==4)continue;for(const segment of spline['Segments']){if(segment['CX'].indexOf(treeX)===-1)continue;if(segment['CY'].indexOf(treeY)===-1)continue;if(segment['LocationCenter']['X']<tree[0]-6000){continue;}
...
(err){}
console.log(minDistanceToSomething);return minDistanceToSomething;}
